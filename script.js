...
function saveUserProfile() {
    ...
    // UI Update karo (Page reload ki zarurat nahi)
    updateProfileUI();
...
function updateProfileUI() {
    ...
    const menuEmail = document.getElementById('menuUserEmail'); // Sidebar wala
    
    // Inputs fill karo agar data hai
    if (storedData) {
        ...
        // Display Name Update karo
        const displayName = storedData.name ? storedData.name : 'Guest User';
        
        if(nameDisplay) nameDisplay.innerText = displayName;
        if(menuEmail) menuEmail.innerText = displayName; // Sidebar me bhi naam dikhega
    }

    // Email hamesha login wala dikhao
...
$('#settingsForm')?.addEventListener('submit', async (e)=>{
  ...
  const file = $('#setLogo')?.files?.[0]; if(file) localStorage.setItem(LS.logo, await fileToDataURL(file));
  set(LS.store, store); renderBrand(); broadcastUpdate();
});
